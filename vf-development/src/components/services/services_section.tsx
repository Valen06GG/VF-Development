    ...
    <section id="servicios" className="py-20 bg-black text-white">
      <div className="wrap max-w-6xl mx-auto px-4">
        ...
        <div className="section-head mb-12 reveal">
          <span className="tag text-purple-400 font-semibold tracking-widest text-xs uppercase block mb-3">
            SERVICIOS
          </span>
        ...
        {/* Grilla de Servicios */}
        <div className="grid grid-cols-1 md:grid-cols-3 bg-[#0d0d0e] border border-[#222226] rounded-2xl overflow-hidden divide-y md:divide-y-0 md:divide-x divide-[#222226]">
          {services.map((service, index) => (
            <div
              key={index}
              className={`p-8 flex flex-col justify-between hover:bg-[#121215] transition-colors duration-200 ${
                index >= 3 ? 'md:border-t md:border-[#222226]' : ''
              }`}
            >
              <div>
                <div className="text-2xl mb-6 bg-[#18181c] w-12 h-12 flex items-center justify-center rounded-xl border border-[#2a2a30]">
                  {service.icon}
                </div>
